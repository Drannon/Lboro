...
for i, a in enumerate(hull_control_point_samples):
    hull_Xs, hull_Ys = util.hull_profile_gen(a, util.beam, util.draught, 1000)
...
sample_v_of_Ds = util.volumeOfDisplacement(
    ...
    B=util.beam,
    LPP=LPP_samples,
    hullXs=sampled_hull_X,
    hullYs=sampled_hull_Y,
)

# Calculate wetted areas for all combinations of samples
sample_wetted_areas = util.wettedArea(
    sample_v_of_Ds, util.draught, LPP_samples, rudder_chord_samples, rudder_span_samples
)
...
sample_about_turns = []
sample_traverse_speeds = []

for i in range(n_sample):
    dv_set = [dv[i] for dv in design_variables]
    sample_turning_circles.append(de.computeTurningCircle(dv_set))
    sample_about_turns.append(de.computeAboutTurn60Time(dv_set))
    sample_traverse_speeds.append(de.computeTraverseSpeed(dv_set))
